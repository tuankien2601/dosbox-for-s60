...
public:
	...
	struct sDeviceHeader{
		RealPt	nextDeviceHeader GCC_ATTRIBUTE(packed);
		Bit16u	devAttributes GCC_ATTRIBUTE(packed);
		Bit16u	strategy GCC_ATTRIBUTE(packed);
		Bit16u	interrupt GCC_ATTRIBUTE(packed);
		Bit8u	name[8] GCC_ATTRIBUTE(packed);
		Bit16u  wReserved GCC_ATTRIBUTE(packed);
		Bit8u	driveLetter GCC_ATTRIBUTE(packed);
		Bit8u	numSubUnits GCC_ATTRIBUTE(packed);
...
int CMscdex::AddDrive(Bit16u _drive, char* physicalPath, Bit8u& subUnit)
{
	...
	case 0x00: {	
		...
		cdrom[numDrives] = NULL; // by JOCO new CDROM_Interface_SDL();
