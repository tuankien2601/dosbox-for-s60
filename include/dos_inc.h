...
#include "config.h" // by JOCO for OFFSETOF

#ifdef _MSC_VER
#pragma pack (1)
#endif
struct CommandTail{
  Bit8u count GCC_ATTRIBUTE(packed);				/* number of bytes returned */
  char buffer[127] GCC_ATTRIBUTE(packed);			 /* the buffer itself */
...
union bootSector {
	struct entries {
		Bit8u jump[3] GCC_ATTRIBUTE(packed);
		Bit8u oem_name[8] GCC_ATTRIBUTE(packed);
		Bit16u bytesect GCC_ATTRIBUTE(packed);
		Bit8u sectclust GCC_ATTRIBUTE(packed);
		Bit16u reserve_sect GCC_ATTRIBUTE(packed);
		Bit8u misc[496] GCC_ATTRIBUTE(packed);
	} bootdata GCC_ATTRIBUTE(packed);
	Bit8u rawdata[512] GCC_ATTRIBUTE(packed);
} GCC_ATTRIBUTE(packed);
#ifdef _MSC_VER
#pragma pack ()
#endif
...
private:
	...
	#endif
	struct sPSP {
		Bit8u	exit[2] 	GCC_ATTRIBUTE(packed);			/* CP/M-like exit poimt */
		Bit16u	next_seg 	GCC_ATTRIBUTE(packed);			/* Segment of first byte beyond memory allocated or program */
		Bit8u	fill_1 		GCC_ATTRIBUTE(packed);				/* single char fill */
		Bit8u	far_call 	GCC_ATTRIBUTE(packed);			/* far call opcode */
		RealPt	cpm_entry 	GCC_ATTRIBUTE(packed);			/* CPM Service Request address*/
		RealPt	int_22 		GCC_ATTRIBUTE(packed);				/* Terminate Address */
		RealPt	int_23 		GCC_ATTRIBUTE(packed);				/* Break Address */
		RealPt	int_24 		GCC_ATTRIBUTE(packed);				/* Critical Error Address */
		Bit16u	psp_parent 	GCC_ATTRIBUTE(packed);			/* Parent PSP Segment */
		Bit8u	files[20] 	GCC_ATTRIBUTE(packed);			/* File Table - 0xff is unused */
		Bit16u	environment GCC_ATTRIBUTE(packed);		/* Segment of evironment table */
		RealPt	stack 		GCC_ATTRIBUTE(packed);				/* SS:SP Save point for int 0x21 calls */
		Bit16u	max_files 	GCC_ATTRIBUTE(packed);			/* Maximum open files */
		RealPt	file_table 	GCC_ATTRIBUTE(packed);			/* Pointer to File Table PSP:0x18 */
		RealPt	prev_psp 	GCC_ATTRIBUTE(packed);			/* Pointer to previous PSP */
		Bit8u interim_flag 	GCC_ATTRIBUTE(packed);
		Bit8u truename_flag GCC_ATTRIBUTE(packed);
		Bit16u nn_flags 	GCC_ATTRIBUTE(packed);
		Bit16u dos_version 	GCC_ATTRIBUTE(packed);
		Bit8u	fill_2[14] 	GCC_ATTRIBUTE(packed);			/* Lot's of unused stuff i can't care aboue */
		Bit8u	service[3] 	GCC_ATTRIBUTE(packed);			/* INT 0x21 Service call int 0x21;retf; */
		Bit8u	fill_3[9] 	GCC_ATTRIBUTE(packed);			/* This has some blocks with FCB info */
		Bit8u	fcb1[16] 	GCC_ATTRIBUTE(packed);			/* first FCB */
		Bit8u	fcb2[16] 	GCC_ATTRIBUTE(packed);			/* second FCB */
		Bit8u	fill_4[4] 	GCC_ATTRIBUTE(packed);			/* unused */
		CommandTail cmdtail GCC_ATTRIBUTE(packed);		
	} GCC_ATTRIBUTE(packed);
	#ifdef _MSC_VER
	#pragma pack()
	#endif
...
public:
	...
	#endif
	struct sOverlay {
		Bit16u loadseg GCC_ATTRIBUTE(packed);
		Bit16u relocation GCC_ATTRIBUTE(packed);
	} GCC_ATTRIBUTE(packed);
	struct sExec {
		Bit16u envseg GCC_ATTRIBUTE(packed);
		RealPt cmdtail GCC_ATTRIBUTE(packed);
		RealPt fcb1 GCC_ATTRIBUTE(packed);
		RealPt fcb2 GCC_ATTRIBUTE(packed);
		RealPt initsssp GCC_ATTRIBUTE(packed);
		RealPt initcsip GCC_ATTRIBUTE(packed);
	} GCC_ATTRIBUTE(packed);
	#ifdef _MSC_VER
	#pragma pack()
	#endif
...
public:
	...
	#endif
	struct sDIB {		
		Bit8u	unknown1[4] GCC_ATTRIBUTE(packed);
		Bit16u	magicWord GCC_ATTRIBUTE(packed);			// -0x22 needs to be 1
		Bit8u	unknown2[8] GCC_ATTRIBUTE(packed);
		Bit16u	regCXfrom5e GCC_ATTRIBUTE(packed);		// -0x18 CX from last int21/ah=5e
		Bit16u	countLRUcache GCC_ATTRIBUTE(packed);		// -0x16 LRU counter for FCB caching
		Bit16u	countLRUopens GCC_ATTRIBUTE(packed);		// -0x14 LRU counter for FCB openings
		Bit8u	stuff[6] GCC_ATTRIBUTE(packed);		// -0x12 some stuff, hopefully never used....
		Bit16u	sharingCount GCC_ATTRIBUTE(packed);		// -0x0c sharing retry count
		Bit16u	sharingDelay GCC_ATTRIBUTE(packed);		// -0x0a sharing retry delay
		RealPt	diskBufPtr GCC_ATTRIBUTE(packed);		// -0x08 pointer to disk buffer
		Bit16u	ptrCONinput GCC_ATTRIBUTE(packed);		// -0x04 pointer to con input
		Bit16u	firstMCB GCC_ATTRIBUTE(packed);		// -0x02 first memory control block
		RealPt	firstDPB GCC_ATTRIBUTE(packed);		//  0x00 first drive parameter block
		RealPt	firstFileTable GCC_ATTRIBUTE(packed);		//  0x04 first system file table
		RealPt	activeClock GCC_ATTRIBUTE(packed);		//  0x08 active clock device header
		RealPt	activeCon GCC_ATTRIBUTE(packed);		//  0x0c active console device header
		Bit16u	maxSectorLength GCC_ATTRIBUTE(packed);	//  0x10 maximum bytes per sector of any block device;
		RealPt	diskInfoBuffer GCC_ATTRIBUTE(packed);		//  0x12 pointer to disk info buffer
		RealPt  curDirStructure GCC_ATTRIBUTE(packed);	//  0x16 pointer to current array of directory structure
		RealPt	fcbTable GCC_ATTRIBUTE(packed);		//  0x1a pointer to system FCB table
		Bit16u	protFCBs GCC_ATTRIBUTE(packed);		//  0x1e protected fcbs
		Bit8u	blockDevices GCC_ATTRIBUTE(packed);		//  0x20 installed block devices
		Bit8u	lastdrive GCC_ATTRIBUTE(packed);		//  0x21 lastdrive
		Bit32u	nulNextDriver GCC_ATTRIBUTE(packed);	//  0x22 NUL driver next pointer
		Bit16u	nulAttributes GCC_ATTRIBUTE(packed);	//  0x26 NUL driver aattributes
		Bit32u	nulStrategy GCC_ATTRIBUTE(packed);	//  0x28 NUL driver strategy routine
		Bit8u	nulString[8] GCC_ATTRIBUTE(packed);	//  0x2c NUL driver name string
		Bit8u	joindedDrives GCC_ATTRIBUTE(packed);		//  0x34 joined drives
		Bit16u	specialCodeSeg GCC_ATTRIBUTE(packed);		//  0x35 special code segment
		RealPt  setverPtr GCC_ATTRIBUTE(packed);		//  0x37 pointer to setver
		Bit16u  a20FixOfs GCC_ATTRIBUTE(packed);		//  0x3b a20 fix routine offset
		Bit16u  pspLastIfHMA GCC_ATTRIBUTE(packed);		//  0x3d psp of last program (if dos in hma)
		Bit16u	buffers_x GCC_ATTRIBUTE(packed);		//  0x3f x in BUFFERS x,y
		Bit16u	buffers_y GCC_ATTRIBUTE(packed);		//  0x41 y in BUFFERS x,y
		Bit8u	bootDrive GCC_ATTRIBUTE(packed);		//  0x43 boot drive
		Bit8u	useDwordMov GCC_ATTRIBUTE(packed);		//  0x44 use dword moves
		Bit16u	extendedSize GCC_ATTRIBUTE(packed);		//  0x45 size of extended memory
		Bit32u	diskBufferHeadPt GCC_ATTRIBUTE(packed);	//  0x47 pointer to least-recently used buffer header
		Bit16u	dirtyDiskBuffers GCC_ATTRIBUTE(packed);	//  0x4b number of dirty disk buffers
		Bit32u	lookaheadBufPt GCC_ATTRIBUTE(packed);		//  0x4d pointer to lookahead buffer
		Bit16u	lookaheadBufNumber GCC_ATTRIBUTE(packed);		//  0x51 number of lookahead buffers
		Bit8u	bufferLocation GCC_ATTRIBUTE(packed);			//  0x53 workspace buffer location
		Bit32u	workspaceBuffer GCC_ATTRIBUTE(packed);		//  0x54 pointer to workspace buffer
		Bit8u	unknown3[11] GCC_ATTRIBUTE(packed);			//  0x58
		Bit8u	chainingUMB GCC_ATTRIBUTE(packed);			//  0x63 bit0: UMB chain linked to MCB chain
		Bit16u	minMemForExec GCC_ATTRIBUTE(packed);			//  0x64 minimum paragraphs needed for current program
		Bit16u	startOfUMBChain GCC_ATTRIBUTE(packed);		//  0x66 segment of first UMB-MCB
		Bit16u	memAllocScanStart GCC_ATTRIBUTE(packed);		//  0x68 start paragraph for memory allocation
	} GCC_ATTRIBUTE(packed);
	#ifdef _MSC_VER
	#pragma pack ()
	#endif
...
private:
	...
	#endif
	struct sDTA {
		Bit8u sdrive GCC_ATTRIBUTE(packed);						/* The Drive the search is taking place */
		Bit8u sname[8] GCC_ATTRIBUTE(packed);						/* The Search pattern for the filename */		
		Bit8u sext[3] GCC_ATTRIBUTE(packed);						/* The Search pattern for the extenstion */
		Bit8u sattr GCC_ATTRIBUTE(packed);						/* The Attributes that need to be found */
		Bit16u dirID GCC_ATTRIBUTE(packed);						/* custom: dir-search ID for multiple searches at the same time */
		Bit16u dirCluster GCC_ATTRIBUTE(packed);					/* custom (drive_fat only): cluster number for multiple searches at the same time */
		Bit8u fill[4] GCC_ATTRIBUTE(packed);
		Bit8u attr GCC_ATTRIBUTE(packed);
		Bit16u time GCC_ATTRIBUTE(packed);
		Bit16u date GCC_ATTRIBUTE(packed);
		Bit32u size GCC_ATTRIBUTE(packed);
		char name[DOS_NAMELENGTH_ASCII] GCC_ATTRIBUTE(packed);
	} GCC_ATTRIBUTE(packed);
	#ifdef _MSC_VER
	#pragma pack()
	#endif
...
private:
	...
	#endif
	struct sFCB {
		Bit8u drive GCC_ATTRIBUTE(packed);			/* Drive number 0=default, 1=A, etc */
		Bit8u filename[8] GCC_ATTRIBUTE(packed);		/* Space padded name */
		Bit8u ext[3] GCC_ATTRIBUTE(packed);			/* Space padded extension */
		Bit16u cur_block GCC_ATTRIBUTE(packed);		/* Current Block */
		Bit16u rec_size GCC_ATTRIBUTE(packed);		/* Logical record size */
		Bit32u filesize GCC_ATTRIBUTE(packed);		/* File Size */
		Bit16u date GCC_ATTRIBUTE(packed);
		Bit16u time GCC_ATTRIBUTE(packed);
		/* Reserved Block should be 8 bytes */
		Bit8u sft_entries GCC_ATTRIBUTE(packed);
		Bit8u share_attributes GCC_ATTRIBUTE(packed);
		Bit8u extra_info GCC_ATTRIBUTE(packed);
		Bit8u file_handle GCC_ATTRIBUTE(packed);
		Bit8u reserved[4] GCC_ATTRIBUTE(packed);
		/* end */
		Bit8u  cur_rec GCC_ATTRIBUTE(packed);			/* Current record in current block */
		Bit32u rndm GCC_ATTRIBUTE(packed);			/* Current relative record number */
	} GCC_ATTRIBUTE(packed);
	#ifdef _MSC_VER
	#pragma pack ()
	#endif
...
private:
	...
	struct sMCB {
		...
		Bit16u psp_segment GCC_ATTRIBUTE(packed);
		Bit16u size GCC_ATTRIBUTE(packed);	
		Bit8u unused[3] GCC_ATTRIBUTE(packed);
		Bit8u filename[8] GCC_ATTRIBUTE(packed);
	} GCC_ATTRIBUTE(packed);
	#ifdef _MSC_VER
	#pragma pack ()
	#endif
...
private:
	...
	#endif
	struct sSDA {
		Bit8u crit_error_flag GCC_ATTRIBUTE(packed);		/* 0x00 Critical Error Flag */
		Bit8u inDOS_flag GCC_ATTRIBUTE(packed);		/* 0x01 InDOS flag (count of active INT 21 calls) */
		Bit8u drive_crit_error GCC_ATTRIBUTE(packed);		/* 0x02 Drive on which current critical error occurred or FFh */
		Bit8u locus_of_last_error GCC_ATTRIBUTE(packed);	/* 0x03 locus of last error */
		Bit16u extended_error_code GCC_ATTRIBUTE(packed);	/* 0x04 extended error code of last error */
		Bit8u suggested_action GCC_ATTRIBUTE(packed);		/* 0x06 suggested action for last error */
		Bit8u error_class GCC_ATTRIBUTE(packed);		/* 0x07 class of last error*/
		Bit32u last_error_pointer GCC_ATTRIBUTE(packed); 	/* 0x08 ES:DI pointer for last error */
		Bit32u current_dta GCC_ATTRIBUTE(packed);		/* 0x0C current DTA (Disk Transfer Address) */
		Bit16u current_psp GCC_ATTRIBUTE(packed); 		/* 0x10 current PSP */
		Bit16u sp_int_23 GCC_ATTRIBUTE(packed);		/* 0x12 stores SP across an INT 23 */
		Bit16u return_code GCC_ATTRIBUTE(packed);		/* 0x14 return code from last process termination (zerod after reading with AH=4Dh) */
		Bit8u current_drive GCC_ATTRIBUTE(packed);		/* 0x16 current drive */
		Bit8u extended_break_flag GCC_ATTRIBUTE(packed); 	/* 0x17 extended break flag */
		Bit8u fill[2] GCC_ATTRIBUTE(packed);			/* 0x18 flag: code page switching || flag: copy of previous byte in case of INT 24 Abort*/
	} GCC_ATTRIBUTE(packed);
	#ifdef _MSC_VER
	#pragma pack()
	#endif
