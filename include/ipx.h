...
struct PackedIP {
	Uint32 host GCC_ATTRIBUTE(packed);
	Uint16 port GCC_ATTRIBUTE(packed);
} GCC_ATTRIBUTE(packed);

struct nodeType {
	Uint8 node[6] GCC_ATTRIBUTE(packed);
} GCC_ATTRIBUTE(packed) ;

struct IPXHeader {
	Uint8 checkSum[2] GCC_ATTRIBUTE(packed);
	Uint8 length[2] GCC_ATTRIBUTE(packed);
	Uint8 transControl GCC_ATTRIBUTE(packed); // Transport control
	Uint8 pType GCC_ATTRIBUTE(packed); // Packet type

	struct transport {
		Uint8 network[4]; GCC_ATTRIBUTE(packed)
		union addrtype {
			nodeType byNode GCC_ATTRIBUTE(packed);
			PackedIP byIP  GCC_ATTRIBUTE(packed);
		} GCC_ATTRIBUTE(packed) addr GCC_ATTRIBUTE(packed);
		Uint8 socket[2] GCC_ATTRIBUTE(packed);
	} dest GCC_ATTRIBUTE(packed), src GCC_ATTRIBUTE(packed);
} GCC_ATTRIBUTE(packed);
