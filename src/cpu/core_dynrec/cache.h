...
#else
			int iSize = CACHE_TOTAL+CACHE_MAXSIZE+PAGESIZE_TEMP-1+PAGESIZE_TEMP;
			cache_code_start_ptr = NULL;
			if(KErrNone == iChunk.CreateLocalCode(iSize,iSize+4096)) //seems like i need some safetymargin over iSize, or Alloc will fail
				{
				iHeap = UserHeap::ChunkHeap(iChunk,iSize,1,iSize+4096);
				if(iHeap!=NULL)
					{
					cache_code_start_ptr=(Bit8u*)iHeap->Alloc(iSize);
					}
				}
			//cache_code_start_ptr=(Bit8u*)malloc(CACHE_TOTAL+CACHE_MAXSIZE+PAGESIZE_TEMP-1+PAGESIZE_TEMP);
#endif
...
// by JOCO my best guess on how to clean up properly
	iHeap->Free( cache_code_start_ptr );
	iHeap->Close();
	iChunk.Close();
/*	for (;;) {
		if (cache.used_pages) {
