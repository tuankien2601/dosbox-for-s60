...
struct Dynamic_Functionality {
	RealPt static_table GCC_ATTRIBUTE(packed);		/* 00h   DWORD  address of static functionality table */
	Bit8u cur_mode GCC_ATTRIBUTE(packed);				/* 04h   BYTE   video mode in effect */
	Bit16u num_cols GCC_ATTRIBUTE(packed);			/* 05h   WORD   number of columns */
	Bit16u regen_size GCC_ATTRIBUTE(packed);			/* 07h   WORD   length of regen buffer in bytes */
	Bit16u regen_start GCC_ATTRIBUTE(packed);			/* 09h   WORD   starting address of regen buffer*/
	Bit16u cursor_pos[8] GCC_ATTRIBUTE(packed);		/* 0Bh   WORD   cursor position for page 0-7 */
	Bit16u cursor_type GCC_ATTRIBUTE(packed);			/* 1Bh   WORD   cursor type */
	Bit8u active_page GCC_ATTRIBUTE(packed);			/* 1Dh   BYTE   active display page */
	Bit16u crtc_address GCC_ATTRIBUTE(packed);		/* 1Eh   WORD   CRTC port address */
	Bit8u reg_3x8 GCC_ATTRIBUTE(packed);				/* 20h   BYTE   current setting of register (3?8) */
	Bit8u reg_3x9 GCC_ATTRIBUTE(packed);				/* 21h   BYTE   current setting of register (3?9) */
	Bit8u num_rows GCC_ATTRIBUTE(packed);				/* 22h   BYTE   number of rows */
	Bit16u bytes_per_char GCC_ATTRIBUTE(packed);		/* 23h   WORD   bytes/character */
	Bit8u dcc GCC_ATTRIBUTE(packed);					/* 25h   BYTE   display combination code of active display */
	Bit8u dcc_alternate GCC_ATTRIBUTE(packed);		/* 26h   BYTE   DCC of alternate display */
	Bit16u num_colors GCC_ATTRIBUTE(packed);			/* 27h   WORD   number of colors supported in current mode */
	Bit8u num_pages GCC_ATTRIBUTE(packed);			/* 29h   BYTE   number of pages supported in current mode */
	Bit8u num_scanlines GCC_ATTRIBUTE(packed);		/* 2Ah   BYTE   number of scan lines active mode (0,1,2,3) = (200,350,400,480) */
	Bit8u pri_char_block GCC_ATTRIBUTE(packed);		/* 2Bh   BYTE   primary character block */
	Bit8u sec_char_block GCC_ATTRIBUTE(packed);		/* 2Ch   BYTE   secondary character block */
	Bit8u misc_flags GCC_ATTRIBUTE(packed);			/* 2Dh   BYTE   miscellaneous flags
									...
									7 reserved
								*/
	Bit8u reserved1[3] GCC_ATTRIBUTE(packed);			/* 2Eh  3 BYTEs reserved (00h) */
	Bit8u vid_mem GCC_ATTRIBUTE(packed);				/* 31h   BYTE   video memory available 00h = 64K, 01h = 128K, 02h = 192K, 03h = 256K */
	Bit8u savep_state_flag GCC_ATTRIBUTE(packed);		/* 32h   BYTE   save pointer state flags 
									...
									7 reserved
								*/
	Bit8u reserved2[13] GCC_ATTRIBUTE(packed);		/*  33h 13 BYTEs reserved (00h) */
