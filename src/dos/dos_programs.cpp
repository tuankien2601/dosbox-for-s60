...
// Nothing to do here.
			...
			if (failed) {
#else
			const char* muuu= temp_line.c_str();
			if (stat(temp_line.c_str(),&test)) {
				failed = true;
...
extern int LoadPremapperFile(const char* premapfilename);

class PREMAP : public Program {
public:
	void Run(void)
	{
		if (cmd->FindCommand(1,temp_line)) {
			if (cmd->FindString("?",temp_line,false)) {
				WriteOut("usage:\nS60MAP [mapfile]\n");
				WriteOut("specify full system - not dos tree - path!\n");
			} else {
				int result = LoadPremapperFile(temp_line.c_str());
			}
		} else {
			/* no parameter in the command line, just output codepage info */
			WriteOut("usage:\nPREMAP [mapfilename]\n");
...
static void PREMAP_ProgramStart(Program * * make) {
	*make=new PREMAP;
}
void DOS_SetupPrograms(void) {
	...
	PROGRAMS_MakeFile("S60MAP.COM", PREMAP_ProgramStart);
}
