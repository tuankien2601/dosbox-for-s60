...
public:
	CStickBindGroup(Bitu _stick,Bitu _emustick,bool _dummy=false) : CBindGroup (){
		...
		sdl_joystick=NULL;//SDL_JoystickOpen(_stick);
		if (sdl_joystick==NULL) {
			button_wrap=emulated_buttons;
			return;
		}

		axes=0;//SDL_JoystickNumAxes(sdl_joystick);
		buttons=0;//SDL_JoystickNumButtons(sdl_joystick);
		hats=0;//SDL_JoystickNumHats(sdl_joystick);
		button_wrap=buttons;
		button_cap=buttons;
		...
		LOG_MSG("Using joystick %s with %d axes, %d buttons and %d hat(s)","dud"/*SDL_JoystickName(stick)*/,axes,buttons,hats);
	}
	~CStickBindGroup() {
	...
	void ActivateJoystickBoundEvents() {
		...
		for (i=0; i<button_cap; i++) {
			if (0/*SDL_JoystickGetButton(sdl_joystick,i)*/)
				button_pressed[i % button_wrap]=true;
		}
		...
		for (i=0; i<axes_cap; i++) {
			Sint16 caxis_pos=0;//SDL_JoystickGetAxis(sdl_joystick,i);
		...
		for (i=0; i<hats_cap; i++) {
			Uint8 chat_state=0;//SDL_JoystickGetHat(sdl_joystick,i);
...
private:
	...
	const char * BindStart(void) {
		if (sdl_joystick!=NULL) return "dud";//SDL_JoystickName(stick);
...
static void InitializeJoysticks(void) {
	...
	if (joytype != JOY_NONE) {
		mapper.sticks.num=0;//SDL_NumJoysticks();
		if (joytype==JOY_AUTO) {
