...
void DOS_Shell::Run(void) {
	...
	//by JOCO
	if( ! g_bJHighResShell )
		WriteOut(MSG_Get("SHELL_STARTUP_S60"),VERSION);
	else
		{
	
		if(machine != MCH_HERC) 
			{ //Hide it for hercules as that looks too weird
...
void SHELL_Init() {
	...
	//by JOCO modded to for MCGA screen
	MSG_Add("SHELL_STARTUP_S60",
			...
			"\xBA Welcome to DOSBox v%-8s \xBA\n"
			"\xBA ported to s60 by kolijoco   \xBA\n"
...
// by JOCO		
		"\xBA        ported to s60 by \033[33mkolijoco\033[37m                                   \xBA\n"
// end JOCO		
		"\xBA                                                                    \xBA\n"
